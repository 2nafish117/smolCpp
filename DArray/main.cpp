...
namespace tf
{
    ...
    inline s32 DArraya_At(DArray* da, u32 index)
    {
    ...
    inline void DArraya_SetAt(DArray* da, u32 index, s32 val)
    {
        assert(index < da->_size && "[DArraya_SetAt] index greater than _size");
        da->_data[index] = val;
    }

    inline u32 DArraya_Size(DArray* da)
    {
        assert((da != nullptr) && "[DArraya_Size] da is null");
        return da->_size;
    }

    inline u32 DArraya_ReservedSize(DArray* da)
    {
